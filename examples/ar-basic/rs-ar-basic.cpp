...
int main(int argc, char * argv[]) try
{
	...
	while (app)
	{
		...
		for (size_t i = 1; i < object_in_sensor.size(); ++i)
		{
			...
			auto points_in_sensor = raster_line(object_in_sensor[0], object_in_sensor[i], 0.01);
...
std::vector<point3_d> raster_line(const point3_d& a, const point3_d& b, float step)
{
	...
	const int n_points = static_cast<int>(distance) / step + 1;
