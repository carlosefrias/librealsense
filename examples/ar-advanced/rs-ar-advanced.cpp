...
struct point3_d {
	...
	point3_d(const float x, const float y, const float z) : f{x, y, z} {}
...
static std::vector<point3_d> raster_line(const point3_d& a, const point3_d& b, float step);
...
int main(int argc, char * argv[]) try
{
	...
	auto object_pose_in_world_initialized = false;
	...
	const auto fisheye_sensor_idx = 1;
	...
	auto intrinsics = fisheye_stream.as<rs2::video_stream_profile>().get_intrinsics();

	auto pose_to_fisheye_extrinsics = pipe_profile.get_stream(RS2_STREAM_POSE).get_extrinsics_to(fisheye_stream);
	...
	while (app)
	{
		...
		auto world_pose_in_device = pose_inverse(device_pose_in_world);
		auto object_pose_in_device = pose_multiply(world_pose_in_device, object_pose_in_world);

		// Get the object vertices in device coordinates
		auto object_in_device = convert_object_coordinates(virtual_object, object_pose_in_device);
		...
		for (size_t i = 1; i < object_in_sensor.size(); ++i)
		{
			...
			auto points_in_sensor = raster_line(object_in_sensor[0], object_in_sensor[i], 0.01f);
		...
		render_text(static_cast<int>(app.height()), device_pose_in_world.tracker_confidence > 2 ?
...
rs2_pose reset_object_pose(const rs2_pose& device_pose_in_world)
{
	...
	const auto object_pose_in_world = pose_multiply(device_pose_in_world, object_pose_in_device);
...
{
	const rs2_quaternion v_as_quaternion = { v.x, v.y, v.z, 0 };
	const auto rotated_v = quaternion_multiply(quaternion_multiply(q, v_as_quaternion), quaternion_conjugate(q));
...
std::vector<point3_d> raster_line(const point3_d& a, const point3_d& b, const float step)
{
	rs2_vector direction = { b.x() - a.x(), b.y() - a.y(), b.z() - a.z() };
	const auto distance = std::sqrt(direction.x*direction.x + direction.y*direction.y + direction.z*direction.z);
	const auto npoints = static_cast<int>(distance / step + 1);

	std::vector<point3_d> points;
	if (npoints > 0)
	{
...
void render_line(const std::vector<pixel>& line, const int color_code)
{
	if (!line.empty())
	{
...
void render_text(const int win_height, const std::string& text)
{
	GLfloat current_color[4];
	glGetFloatv(GL_CURRENT_COLOR, current_color);
	glColor3f(0, 0.5, 1);
